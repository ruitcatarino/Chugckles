...
const CreateGame = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!gameName || selectedDecks.length === 0 || players.filter(p => p.trim()).length === 0) {
            alert('Please enter a game name, select at least one deck, and add at least one player.');
            return;
        }

        try {
            const message = await createGame(gameName, selectedDecks, players.filter(p => p.trim()), rounds);
            console.log(message);
            navigate(`/games-list`);
        } catch (error) {
